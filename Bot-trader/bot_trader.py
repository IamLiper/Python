...
# Se você mudar para 2h (120s), talvez queira checar com mais frequência. Mantendo 5 min (300s) por enquanto.
...
def execute_buy_order(symbol, usdt_amount_to_spend, symbol_details, current_price):
    ...
    try:
        ...
        # Mensagem de Telegram ANTES de enviar a ordem
        ...
        if fills:
            ...
            # ====== ADICIONADO: Mensagem de Telegram para SUCESSO na COMPRA ======
...
def execute_sell_order(symbol, quantity_to_sell, symbol_details, current_price):
    ...
    try:
        ...
        if fills:
            ...
            print(f"[{symbol}] ✅ VENDA Executada (Saída de Long). Pool Receita atualizada: {total_revenue:.2f}.") # Loga receita DESSA venda
            # ====== ADICIONADO: Mensagem de Telegram para SUCESSO na VENDA ======
...
def initialize_trading_state():
    ...
    # AJUSTADO PARA VALORES DE EXEMPLO PARA O INTERVALO DE 2 HORAS
    # ESTES PARÂMETROS PRECISAM SER OTIMIZADOS/TESTADOS PARA UM DESEMPENHO REAL.
    filtered_strategy_params = {
        # Exemplo de ajuste: períodos de EMA reduzidos, thresholds ligeiramente ajustados
        'BTCUSDT': {'fast_period': 10, 'medium_period': 25, 'slow_period': 50, 'adx_period': 14, 'adx_threshold': 20, 'rsi_period': 14, 'rsi_overbought': 70, 'rsi_oversold': 30, 'volume_length_short': 20, 'volume_length_long': 60, 'volume_threshold': 1.0, 'atr_period': 14},
        'ETHUSDT': {'fast_period': 12, 'medium_period': 30, 'slow_period': 60, 'adx_period': 14, 'adx_threshold': 25, 'rsi_period': 14, 'rsi_overbought': 75, 'rsi_oversold': 35, 'volume_length_short': 20, 'volume_length_long': 60, 'volume_threshold': 1.2, 'atr_period': 14}, # Exemplo: parâmetros diferentes
        'SOLUSDT': {'fast_period': 8, 'medium_period': 20, 'slow_period': 40, 'adx_period': 14, 'adx_threshold': 15, 'rsi_period': 14, 'rsi_overbought': 65, 'rsi_oversold': 25, 'volume_length_short': 20, 'volume_length_long': 60, 'volume_threshold': 0.8, 'atr_period': 14}, # Exemplo: parâmetros diferentes
    }


    global LIMITE_KLINES
    LIMITE_KLINES = 0 # Reseta para recalcular com base no maior requisito das estratégias escolhidas
                    ...
                    if not state['holding'] and not buy_executed_in_this_iteration and current_total_usdt_free >= symbol_details['min_notional']:
                        ...
                        if usdt_amount_to_spend >= symbol_details['min_notional']:
                             ...
                             if qty is not None and price is not None: # Se a ordem de compra foi executada com sucesso
                                 ...
                                 # Telegram já é enviado dentro de execute_buy_order (agora incluindo o de sucesso)
                             else:
                ...
                # Se a estratégia sinalizou VENDA ('SELL')
                    ...
                    if state['holding'] and quote_asset_balance >= symbol_details['min_quantity']:
                        ...
                        if qty is not None and price is not None and revenue is not None: # Se a ordem de venda foi executada com sucesso
                             ...
                             state['usdt_pool_revenue'] += revenue # Soma a receita desta venda à pool do par
                             ...
                             if state['buy_price'] > 0: # Verifica se tínhamos um preço de compra registrado da última compra
                                 ...
                                 # Mensagem de Telegram para P/L (já existia)
                             ...
                             # Telegram já é enviado dentro de execute_sell_order (agora incluindo o de sucesso)
                        else:
